...
bn_t eGCD(const bn_t N, const bn_t a, const bn_t b, bn_t &x, bn_t &y)
{
  ...
  {
    bn_t r = eGCD(N, b, a % b, x, y); /* r = GCD(a, b) = GCD(b, a%b) */
    bn_t t = x;
    x = y;
    MODULO(N) y = t - a / b * y;
    return r;
...
error_t leath_create_paillier_t::peer1_step1(mem_t session_id, crypto::paillier_t paillier, message1_t& out)
{
...
error_t leath_create_paillier_t::peer2_step1(leath_server_share_t& server_share, mem_t session_id, int server_id, ecc_point_t pk, bn_t sk, bn_t h_1, bn_t h_2, bn_t _N, const message1_t& in, message2_t& out)
{
  error_t rv = 0;
...
// if all good, prepare return message
  ...
  bn_t _c_i;
  _c_i = paillier.add_ciphers(paillier.mul_scalar(in.c_1, sk), paillier.encrypt(r, r_r));

  out.pk_i = pk;
  out._c_i = _c_i;
  out.zk_pdl_mult.p(curve, pk, in.c_1, _c_i, paillier, h_1, h_2, _N, session_id, 1, sk, r, r_r);

  // store locally
  server_share.h_1 = h_1;
  server_share.h_2 = h_2;
  server_share._N = _N;
  server_share.N = in.N;
  int bits = curve.bits();
...
error_t leath_create_paillier_t::peer1_step2(leath_client_share_t& client_share, mem_t session_id, int server_id, bn_t c_1, bn_t h_1, bn_t h_2, bn_t _N, const message2_t& in)
{
  error_t rv = 0;
